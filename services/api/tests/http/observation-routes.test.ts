/**
 * Full HTTP-level contract tests for the observation routes: the real
 ...
 * `tests/integration/observations-history.test.ts`. No `If-Match` case here:
 * `observation` carries no revision, so no route in this tag needs one.
 *
...
import type { ApiError, Garden as GardenResource, Observation } from '@verdery/api-contracts';
import type {
  DatabaseGateway,
  DatabaseSchema,
...
const SUITE_NAME = 'observation routes (HTTP)';
...
describe.skipIf(!dockerAvailable)(SUITE_NAME, () => {
  ...
  it('rejects recording an observation missing the Idempotency-Key header with 400', async () => {
    ...
    const response = await app.inject({
      ...
      payload: { noteText: 'Looks healthy.' },
    });

    expect(response.statusCode).toBe(400);
  });

  it('reports correcting an observation that does not exist as 404', async () => {
    const token = randomUUID();
    tokenVerifier.registerIdToken(token, randomUUID());

    const response = await app.inject({
      method: 'POST',
      url: `/v1/observations/${generateUuidV7()}/corrections`,
      headers: { ...bearer(token), 'idempotency-key': generateUuidV7() },
      payload: { correctionKind: 'amendment', noteText: 'Correction text.' },
    });

    expect(response.statusCode).toBe(404);
    expect(asError(response).error.code).toBe('observation.not_found');
  });

  it('runs the full lifecycle over real HTTP: record, list, correct', async () => {
    const { token, garden } = await createGardenAsOwner();
    ...
    const recorded = await app.inject({
      ...
      url: `/v1/gardens/${garden.id}/observations`,
      headers: { ...bearer(token), 'idempotency-key': generateUuidV7() },
      payload: { noteText: 'First growth spurt.', conditionSummary: 'Vigorous' },
    });
    expect(recorded.statusCode).toBe(201);
    const observation = asObservation(recorded);
    expect(observation).toMatchObject({
      gardenId: garden.id,
      noteText: 'First growth spurt.',
      conditionSummary: 'Vigorous',
      isCorrected: false,
      correctionKind: null,
    });
    ...
    const { items } = asItems<Observation>(listed);
    expect(items).toHaveLength(1);
    expect(items[0]?.id).toBe(observation.id);
    ...
    const correction = asObservation(corrected);
