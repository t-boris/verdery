...
describe.skipIf(!dockerAvailable)(SUITE_NAME, () => {
  ...
  it('rolls back, leaving the identity-and-gardens-baseline schemas and tables otherwise intact', async () => {
    ...
    // `count: 20` undoes this migration and every migration applied after it
    // (currently plants-observations-tasks-baseline through
    // service-organizations-and-client-engagements, each of which depends,
    // directly or transitively, on tables this one creates and must come down
    // first). Update this count when a later migration is added on top.
    await runner({
      databaseUrl,
      dir: MIGRATIONS_DIRECTORY,
      direction: 'down',
