...
describe.skipIf(!dockerAvailable)(SUITE_NAME, () => {
  ...
  it('rolls back, leaving the identity-and-gardens-baseline schemas and tables otherwise intact', async () => {
    ...
    // (currently plants-observations-tasks-baseline and search-indexes, each
    // of which depends, directly or transitively, on tables this one creates
    // and must come down first). Update this count when a later migration is
    // added on top.
    await runner({
      databaseUrl,
      dir: MIGRATIONS_DIRECTORY,
