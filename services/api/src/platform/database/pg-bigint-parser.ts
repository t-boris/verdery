 ...
 * the full int8 range. Every counter-shaped `bigint` column this service
 * defines (`revision` on `profile`/`garden`/`membership`, `platform.
 * sync_change.sequence`) increments by one per accepted mutation and
 * cannot approach `Number.MAX_SAFE_INTEGER` (2^53) within the service's
 * realistic lifetime. `media.media_record.declared_byte_size`/
 * `verified_byte_size` and `media.quota_reservation.reserved_bytes`
 * (1785100000000_media-lifecycle-and-quotas.sql) are `bigint` for a
 * different reason — file sizes, not counters — but stay just as safe
 * under this same parser: even a multi-terabyte file is many orders of
 * magnitude below 2^53. The Kysely row types already declare every one of
 * these columns as `number`, matching this parser, not the driver's own
 * default.
 *
