 ...
 * (architecture/backend-modular-monolith.md section "15. Idempotency") — the
 * same "each module binds the pattern to its own unit of work" reasoning
...
export async function runIdempotentCommand<T>(
  ...
  work: (context: DeletionTransactionContext) => Promise<T>,
): Promise<T> {
