...
describe('createObservationPhoto', () => {
  it('builds an observation photo stamped with the injected clock', () => {
    expect(createObservationPhoto(PHOTO_ID, OBSERVATION_ID, MEDIA_ID, 'leaf_front', NOW)).toEqual({
      id: PHOTO_ID,
  ...
  it('rejects an unrecognized purpose', () => {
    expect(() =>
      createObservationPhoto(PHOTO_ID, OBSERVATION_ID, MEDIA_ID, 'close_up', NOW),
