...
export class FakeWeatherRecordRepository implements WeatherRecordRepository {
  ...
  /** Effective order, mirroring the Kysely adapter — see `WeatherRecordRepository.findLatestObservation`. */
...
/**
 * In-memory `GeoreferenceReader` (P9D-SEASON-DATA-01) — one optional
...
// The plants-inventory read doubles live in their own file for the
// 600-line budget; re-exported so every existing import keeps working.
export * from './seasonal-test-doubles.js';
