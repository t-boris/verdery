/**
 * Shared shape for every tasks-recommendations command handler: check for a
 ...
 * `application/run-idempotent-command.ts` implements, and every sibling
 * Phase 4 module (`media`, `observations-history`, `plants-inventory`) has
 * already made its own copy of: the pattern is shared architecture
 ...
 * `TasksRecommendationsUnitOfWork`/`TasksRecommendationsTransactionContext`,
...
export async function runIdempotentCommand<T>(
  ...
  unitOfWork: TasksRecommendationsUnitOfWork,
  input: IdempotencyRecordInput,
  responseStatusCode: number,
  work: (context: TasksRecommendationsTransactionContext) => Promise<T>,
): Promise<T> {
