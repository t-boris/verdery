/**
 * Shared shape for every plants-inventory command handler: check for a prior
 ...
 * `application/run-idempotent-command.ts` implements, and `media` already
 * made its own copy of (`media/application/run-idempotent-command.ts`): the
 * pattern is shared architecture (architecture/backend-modular-monolith.md,
 * section "15. Idempotency"), but its `unitOfWork`/`work` parameters are
 * bound to this module's own `PlantsInventoryUnitOfWork`/
 * `PlantsInventoryTransactionContext`, and gardens-mapping does not export
 * its copy through `public.ts` — it is an internal implementation detail of
 * that module's own commands, so this module cannot import it and instead
 * follows the same shape locally.
 *
...
export async function runIdempotentCommand<T>(
  ...
  unitOfWork: PlantsInventoryUnitOfWork,
  input: IdempotencyRecordInput,
  responseStatusCode: number,
  work: (context: PlantsInventoryTransactionContext) => Promise<T>,
): Promise<T> {
  ...
  try {
    ...
    // idempotency record's own primary key: every row a command in this
    // module writes uses a freshly generated UUIDv7, or is guarded by the
    // capability check that already ran. A true concurrent duplicate request
    // lost the race to `save`; the winner's result, committed under the same
