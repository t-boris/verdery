 ...
 *
 * Reviewed facts and licensed imagery have independent lifecycles. `profile`
 * is therefore null when no fact projection exists, while imagery is loaded
 * cache-aside through `TaxonImageEnricher` and can still be returned.
 */
...
export class GetTaxonProfile {
  ...
  async execute(taxonomyReferenceId: Uuid): Promise<PlantTaxonProfileResult> {
    ...
    if (images.length === 0) {
      await this.imageEnricher.enrich(taxonomyReferenceId);
