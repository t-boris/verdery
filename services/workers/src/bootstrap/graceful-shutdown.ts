 ...
 * must stop scheduling new relay ticks, let any in-flight tick finish, and
 * close the database pool inside that window.
 *
 * Deliberately duplicated from `services/api/src/bootstrap/graceful-
 * shutdown.ts` rather than shared: this package does not import the running
 * API application — architecture/backend-modular-monolith.md section
 * "19. Worker Boundary".
 *
...
/** Runs the shutdown sequence under a deadline. A drain that outlives the grace period is reported, not awaited forever. */
