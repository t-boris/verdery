...
| `POST .../media` (registration)     | 60/hour                       | A heavy capture session is tens of photos. 60/hour bounds `T-COST-03`'s storage-exhaustion vector at 60 × 25 MiB = 1.5 GiB/hour per profile even before the storage ceiling below.                                                                                                                        |
...
| ------------------------ | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Stored bytes per account | `PROPOSED` **5 GiB**  | `garden_photo` is capped at 25 MiB, so 5 GiB is ~200 full-size photos plus plans — comfortably beyond a residential garden's photo history over a season, and a number whose worst case is a few dollars per account per month rather than an unbounded bill. Enforced by summing `quota_reservation` at registration, which the existing ledger already supports. |
...
| Raw capture (`raw_capture`) | 30 days after extraction    | **No**   | `RAW_CAPTURE_RETENTION_DAYS = 30`, `enforced: false`, and structurally unreachable: `deriveDefaultRetentionDeadline` returns `null` for every class except `export_package`, so no raw-capture deadline is ever computed. The anchoring event has no producer until Garden Scan (Phase 10). There is also no bucket lifecycle rule on `verdery-dev-raw-capture`. **This is user-visible policy the system does not implement.** |
