...
| ----------------- | ------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| **Weather**       | `WEATHER_ACTIVE_PROVIDER_KEY` (`open-meteo` in dev and prod) | Open-Meteo, selected by `P0-PROV-01` and now ACTIVE — set from each environment's own `VERDERY_WEATHER_ACTIVE_PROVIDER_KEY` | Unsetting it returns the typed `noProviderConfigured` degradation; `weather.refresh_sweep_completed` counts it |
