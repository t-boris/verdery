...
| **Aerial tracing kill-switch**  | `AERIAL_TRACE_AI_ENABLED=false`. No aerial vision adapter is constructed; the endpoint returns `disabled` before imagery, quota, or Vertex calls. Development enables this experimentally; staging/production remain default-off until the representative fixture evaluation passes.                                                                                               | API redeploy                       | All users, all gardens  |
