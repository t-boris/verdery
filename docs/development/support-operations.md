...
| Surface and version              | Web or iOS, and the build. `SERVICE_VERSION` now reports the deployed image's tag (the commit SHA), so the server half identifies the exact build that served the request.                                    |
