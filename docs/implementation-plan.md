...
| P4     | P4-WEB-01 and the P4-SEARCH-01 backend implemented, rest of P4 not reassessed | The web client for plants, observations, and manual tasks (`P4-WEB-01`): `plant-gateway.ts`/`observation-gateway.ts`/`task-gateway.ts` and TanStack Query hooks covering every operation under the `Plants`, `Observations`, and `Tasks` contract tags; forms and lists for every command not blocked on file upload; English/Russian localization; unit tests for gateway request-shaping and enum-label mapping. The P4-SEARCH-01 backend (`services/api/`): PostgreSQL `pg_trgm` trigram search and structured filters, verified against real PostgreSQL — `GET /gardens/{gardenId}/plants` (`SearchPlants`: trigram-fuzzy `displayName` matching, `lifecycleStage`/`status`/`groupingKind` filters, cursor pagination matching `ListGardens`'s own shape); `SearchTaxonomyReferences` upgraded from plain `ILIKE` substring matching to `pg_trgm` trigram similarity; a trigram `nameQuery` filter added to `ListGardens`, backward compatible with every existing caller | The contract now has a `GET /gardens/{gardenId}/plants` list operation (P4-SEARCH-01), but no client calls it yet: the web plants page still navigates to a known plant id rather than listing an inventory, and photo-identification, photo-attachment, and file-attachment commands remain implemented at the gateway layer only and left out of the UI, the same media-upload gap `docs/development/deferred-capabilities.md` documents; `P4-IOS-01`, `P4-DESIGN-01`, `P4-OBS-01`, and `P4-QA-01` are not reassessed by this update; explicit G4 approval is a repository-owner decision |
| P5–P14 | Not started as release phases                                                 | Some Phase 1 geometry primitives and fixtures are reusable foundations                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Phase-specific work packages and gates remain open                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
