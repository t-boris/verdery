...
| P4     | P4-WEB-01 implemented, rest of P4 not reassessed | The web client for plants, observations, and manual tasks (`P4-WEB-01`): `plant-gateway.ts`/`observation-gateway.ts`/`task-gateway.ts` and TanStack Query hooks covering every operation under the `Plants`, `Observations`, and `Tasks` contract tags; forms and lists for every command not blocked on file upload; English/Russian localization; unit tests for gateway request-shaping and enum-label mapping                                                                               | No `GET /gardens/{gardenId}/plants` list operation exists in the contract, so the plants page navigates to a known plant id rather than listing an inventory (see `docs/development/deferred-capabilities.md`); photo-identification, photo-attachment, and file-attachment commands are implemented at the gateway layer only and left out of the UI, the same media-upload gap that section documents; `P4-IOS-01`, `P4-DESIGN-01`, `P4-SEARCH-01`, `P4-OBS-01`, and `P4-QA-01` are not reassessed by this update; explicit G4 approval is a repository-owner decision |
