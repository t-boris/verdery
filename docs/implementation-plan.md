...
- Operational team sharing, professional client delivery, AR, LiDAR, improved vectorization, and
  richer seasonal context are proposed after foundation validation; 3D, future Time Machine
  projections, and conversational assistance are future capabilities, while automated garden
  reconstruction is research-only and has no committed release phase. [Source:
  technical-specification.md, sections "6.2 Proposed Next Scope" and "6.3 Future Scope"]
...
- Every advanced capture mode must have a manual fallback, and generated geometry remains a reviewable proposal until the user accepts it. [Source: architecture/garden-capture-and-scan.md, sections "2. Product Position", "9.5 Acceptance and Synchronization", and "10.13 Promotion Criteria"]
...
- Phases 1–9 are implemented. G2–G8 remain explicit owner-controlled release approvals rather than inferred completions; G9 is approved with its cross-cutting evidence recorded. The development API and web application are deployed, and iOS build 192 has been uploaded to TestFlight for Apple processing. This does not imply GA readiness: staging, production, several enforcement switches, live worker rollout, provider decisions, and owner-operated release evidence remain open. [Source: tasks/todo.md, sections "Phase 1 — Engineering Foundation, complete" through "G9 phase review"; docs/development/ios-distribution.md; repository and deployment evidence reviewed July 27, 2026]

### 2.2 Reliability of the Baseline

| Source                        | Status                                       |                                               Planning reliability | Use in this plan                                                           |
| ----------------------------- | -------------------------------------------- | -----------------------------------------------------------------: | -------------------------------------------------------------------------- |
| `technical-specification.md`  | Draft 1.0; architecture approved             | High for current product intent; Medium for proposed release scope | Requirements, journeys, priorities, risks, open product decisions          |
| `high-level-architecture.md`  | Draft 0.6; approved detailed-design baseline |                                                               High | System context and non-negotiable boundaries                               |
...
| P4      | Implementation complete, G4 pending                                                          | A minimal `media` module; `plants-inventory`, `observations-history`, and `tasks-recommendations` backend modules (plant instances, groups/rows, append-only observations with backward-pointing corrections, gated-lifecycle manual tasks), each with its own revision journal, optimistic concurrency (`Plant`/`Task`; `Observation` has none by design), and idempotency; the full `Plants`/`Observations`/`Tasks` REST contract (`P4-CONTRACT-01`, 24 operations); matching web and iOS clients (`P4-WEB-01`/`P4-IOS-01`) covering every operation not blocked on file upload; `pg_trgm` trigram search and structured filters (`P4-SEARCH-01`) — `GET /gardens/{gardenId}/plants`, upgraded `SearchTaxonomyReferences`, a `nameQuery` filter on `ListGardens`; a UX validation pass across both clients against six required scenarios (`P4-DESIGN-01`) that found and fixed three real iOS gaps (identification invisible on an existing plant, quantity field not gated by grouping kind, observation corrections missing their back-reference); targeted new tests closing genuine gaps in cross-garden task-target authorization and timezone-boundary date parsing (`P4-QA-01`, 425 → 430 backend tests); a real deploy-pipeline defect found and fixed against the live `verdery-dev` database (`pg_trgm` needs database-level `CREATE`, which the least-privilege migration identity did not have)                                                                                                                                                                                                                                                                                                                                                                                                                 | `P4-OBS-01` (privacy-safe product events) is a documented deferral, not built: blocked on `P0-SEC-01`'s still-undecided consent model, and no product-analytics/consent infrastructure exists anywhere in this codebase to build against. `GET /gardens/{gardenId}/plants` exists but neither client calls it yet. Photo-identification/photo-analysis remain honest, always-empty placeholders (no ML service exists). Photo-attachment and file-attachment commands were implemented at the gateway layer only, left out of both clients' UI at the time — the same media-upload gap `docs/development/deferred-capabilities.md` documented; `P6-WEB-01` and `P6-IOS-01` have since wired real upload capability into one attachment point per client (web: garden photo; iOS: plant photo and observation photo), with the remaining attachment points (`AddPlantFromPhoto`/`SetPrimaryPlantPhoto`/`ConfirmPlantIdentification`/`AttachTaskFile`) still gateway-only — see that file's own current account. The `postgis`-on-a-fresh-environment risk this row once named is fixed (Phase 6's `07-iam-database-bootstrap.sh` now installs it defensively via the break-glass superuser). Explicit G4 approval is a repository-owner decision |
| P5      | Implementation complete, G5 pending                                                          | Every backend endpoint of the offline-sync protocol (`platform.sync_change` wired into all 23 mutating commands across 4 modules; versioned client registration, dependency-aware push, incremental pull, acknowledge); the full native engine (`CorePersistence`/`CoreSynchronization`, all five features — Gardens, Map, Plants, Observations, Tasks — routed through atomic local-projection-plus-outbox transactions, a real bounded push/pull engine with backoff and a status model, conflict recovery with a real per-command-type "safely replayable" table, revocation cascade-removal with a named offline-attack test); web continuity bounded to a stale indicator and schema-versioned recoverable drafts for three forms and the map editor; payload-free sync observability with a concrete dashboard/alert-candidate writeup; an 18-item required testing-matrix assessment closing every genuine gap (randomized convergence, clock skew, large backlog, schema upgrade with a pending outbox, process termination). 963 tests across 6 TypeScript/JS workspace packages, 674 tests/93 suites on iOS (both figures CI-confirmed on the pinned toolchain), a real Next.js production build                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Later P6 and P9 work closed the media-sync and collaboration-matrix dependencies and introduced real membership-revocation producers. Web stale/draft treatment remains intentionally limited to selected forms and the map editor rather than every mutation surface. Two P5-QA-01 findings — an authentication/authorization/validation/conflict failure on a whole push call retrying like a transient one, and `P5-CONFLICT-01`'s resolution paths not sharing one transaction — were fixed in a later, explicitly-approved follow-up session (`RemoteSyncEngine.eligiblePending(bypassingAutomaticRetryGate:)`/`retryNow()`; `CorePersistence.SyncTransactionContext`); that follow-up's own honest boundary (the feature-specific local-store write inside conflict resolution stays outside real transaction scope, a bounded/self-healing gap, not data loss) is recorded in `tasks/todo.md`'s Phase 5 "Deferred with reason" table. Explicit G5 approval is a repository-owner decision                                                                                                                                                                                                                                                |
| P6      | Implementation complete, G6 pending                                                          | The full media pipeline against real infrastructure: the grown `media.media_record` model (orthogonal upload/processing state machines, quota reservations, sensitivity classification with derivative inheritance); four live private buckets with verified access controls, lifecycle rules, and browser-upload CORS; the direct-resumable upload API and both clients' real upload flows (web: GCS chunk protocol with IndexedDB resume-after-reload; iOS: background URLSession with durable-before-network local files and relaunch recovery); the transactional-outbox relay and Cloud Tasks two-hop authenticated worker pipeline (`services/workers` became a real HTTP target); real byte-level validation (`file-type`/`image-size`, hand-written PDF safety pass, honest always-`unavailable` malware placeholder, malicious fixture suite); real derivative generation (`sharp` — thumbnails, screen previews, plan high-res, XYZ tile pyramids) with database-enforced idempotency; plan import and similarity-transform calibration (shared hand-computed fixtures reproduced byte-identically by TypeScript and Swift, residuals/RMS with honest null-below-2-points, transform revisions, geometry locking) on both web and iOS; retention/deletion (referenced-media blocking with FOR-SHARE race closure, prefix-scoped byte deletion with re-list verification, 7-day orphan reconciliation, retention-policy endpoint); pipeline observability (structured events closing every section-19 signal incl. deletion lag, 14 log-metric definitions, 6 reasoned alert candidates with runbooks); and a QA audit closing the required matrix with five break-the-check-verified tests. api 120 files/794 tests, workers 20/111, web 57/455, iOS 782 tests/109 suites, geometry-contracts 113 — all CI-confirmed | `P6-PLANT-01` is a documented deferral (blocked on `P0-PROV-01`'s undecided photo-identification provider, the same honest-placeholder posture as Phase 4). PDF page rendering/display, plan tile consumption, geographic anchors, video deep validation (`ffprobe`), and the malware-scanning provider each remain documented deferrals with named blockers. `services/workers` is code-and-script-complete but not yet deployed to Cloud Run (Cloud SQL IAM membership, a real `DATABASE_URL` secret, and CI/CD wiring are the named prerequisites — an explicit repository-owner decision). Explicit G6 approval is a repository-owner decision                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| P7      | Implementation complete, G7 pending                                                          | The full care loop: the recommendation data model with physically-enforced evidence (a candidate cannot commit without its own evidence row) and structural restricted-tier exclusion; the provider-agnostic weather and plant-content integration layers (registry, normalization, freshness/cache/quota/timeout, replacement proven with two fakes, typed no-provider states — real vendors blocked on `P0-PROV-01`); the deterministic versioned rule engine with four launch rules (all marked awaiting horticultural review, structural content-category exclusions, 19 reviewer-readable fixtures); scheduled weather/evaluation/delivery sweeps on the established worker pattern with proven duplicate safety; Today queries and the five feedback/conversion commands with the outcome-history chain proven on real Postgres; both Today clients (iOS `FeatureRecommendations` online-with-honest-degradation; web with a real 9/9 browser care-loop E2E that also fixed the harness broken since P6); the bounded Vertex AI explanation adapter behind a default-off kill-switch with proven rollback and bilingual adversarial fixtures; durable notification intents with DST-proven quiet hours, dedup, expiry, and the FCM delivery worker with send-time access/preference/freshness rechecks and invalid-token handling; the consolidated safety catalog with a lexicon-alignment drift test; server-side care-loop quality measurement with structurally-pinned payload-free events; and a nine-surface QA matrix closed with break-verified tests plus a root-caused fix for the Testcontainers flake (3× consecutive fully-green 191-file runs). api 191/1406, workers 20/118, web 62/518 + 9/9 E2E, iOS 808/114 — all CI-confirmed                                                                         | Real weather/plant-content vendors and photo-ID remain blocked on `P0-PROV-01`; the horticultural sign-off (rules) and human AI-evaluation pass are named human gates the catalog holds open; live Vertex/FCM enablement and client FCM wiring are explicit owner decisions; consented client-side analytics stays blocked on `P0-SEC-01`; runtime Russian explanation generation is deferred with the story documented. Explicit G7 approval (a controlled US private beta) is a repository-owner decision                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| P8      | Implementation complete, G8 pending                                                          | Every one of the sixteen work packages has its buildable half delivered: account/garden export with a frozen consistency boundary; the web client DEPLOYED and redesigned; deletion with 30-day recovery whose completeness is proved against the live database catalog; a 92-row threat model; the iOS app building for a real device (exposing four latent ship-blockers invisible to seven phases of macOS-only compilation) and archiving into a signed `.ipa`; ten exercised runbooks; the production edge and database hardening written and unapplied behind explicit opt-in guards; derived SLOs, a k6 harness, a support design, and a 61-gate GA checklist; accessibility, keyboard, responsive, reduced-motion and localization suites with thirteen real fixes; a fact-checked privacy draft that corrected twelve claims; and an enforceable CSP plus an App Check switch that now emits the very telemetry its flip was waiting on. api 208/1550, web 67/670 + 48 E2E, iOS 825/116                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Everything remaining is an owner decision or action, not unbuilt work: the register signature, four Apple Developer actions, a domain and ~150 USD/month with a downtime window, SLO approval, a staging environment, a support rota, real-device accessibility sign-off, legal review, and the enforcement flips once beta telemetry exists. Two of the eight exit criteria are reachable today; no production project, no staging, and no restore has ever been performed. Explicit G8 approval is a repository-owner decision                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| P9      | Complete; G9 approved                                                                        | Operational collaboration, professional organizations and garden assignments, immutable client publications, a read-only client portal and factual timeline, client-scoped media/export, seasonal context, web and iOS UX, cross-cutting authorization and boundary matrices, development deployment, and TestFlight delivery are recorded in `tasks/todo.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | One low-severity professional-assignment observability gap is recorded; future runtime feature switches remain optional follow-up work                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| P10–P14 | P10 in progress; P11 implementation substantially complete, G10 pending; P12–P14 not started | Earlier plant, geometry, media, observation, search, sharing, and context foundations are reusable. Phase 11 has landed its candidate domain and API, six joined search filters, the purpose-labelled visual journal (capture and ordered comparison sequence) on both clients, typed measurements on both clients, taxon catalog browse with cited profiles on web, shareable web filter state, and publisher pickers that stage derivative-only media. Its evidence report is `docs/development/phase-11-qa-matrix.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | P10 completes real plant identification and property-plan OCR; P11 still owes an accessibility pass on real hardware, performance and cost measurement, near-duplicate detection (needs a perceptual-hashing dependency; the exact-duplicate half is built), and taxon imagery (no read surface exists); time-lapse generation was cut by owner decision on 2026-08-03; field mapping, conversational assistance, 3D, and future-scenario work retain independent gates; automated reconstruction is research-only                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
The advanced product adds constrained conversational assistance, a shared-data 3D view, and
illustrative future Time Machine behavior. Automated multi-capture reconstruction is retained only
as an uncommitted research question. The factual Garden Timeline does not depend on future
simulation and may ship with the P9 client portal. These capabilities must not delay the foundation
release. [Source: technical-specification.md, sections "6.3 Future Scope", "FR-16: Automated Garden
Reconstruction Research", "FR-28: Conversational Assistance", "FR-29: 3D Garden View", and "FR-30:
Time Machine"]
...
### Assumption A-5: Advanced Capture Requires Evidence Gates

- **Explanation:** AR and LiDAR use evidence gates inside Phase 12. Automated reconstruction and model-based recognition remain uncommitted research until a separate promotion decision.
- **Evidence:** The capture design makes each committed capture stage independently shippable and defines separate promotion criteria for automated reconstruction. [Source: architecture/garden-capture-and-scan.md, sections "3. Staged Capability Plan", "10.11 Evaluation", and "10.13 Promotion Criteria"]
- **Confidence:** High.
- **Validation path:** Approve datasets, ground truth, metrics, device matrix, privacy basis, quality thresholds, and unit-cost limits before each evaluated capability ships; require a new ADR and numbered phase before automated reconstruction enters delivery.
...
2. **Manual before automatic.** Manual editing and correction ship before plan extraction, AR proposals, or any future reconstruction proposal.
...
These principles derive from the approved backend, sync, capture, security, provider, cost, testing,
and repository designs. [Source: architecture/backend-modular-monolith.md, sections "5. Module
Shape" and "23. Extraction Criteria"; architecture/offline-synchronization.md, section "4. Authority
Model"; architecture/garden-capture-and-scan.md, sections "9.5 Acceptance and Synchronization" and
"10.4 Candidate Proposal Model"; architecture/external-integrations.md, section "2. Integration
Boundary"; architecture/cost-and-scaling.md, section "18. Scaling Triggers"; AGENTS.md]
...
| WS-PLANT    | Plant intelligence and journal    | Taxonomy, licensed source assertions, candidate suitability, plant search, visual journal, and health-review semantics | Product, horticulture, media, clients, privacy |
...
P8 Foundation beta, hardening, and US general availability
         ...
         │                         │
         │                         ▼
         │                     P11 Plant intelligence, candidates, and visual journal
         │                         │
         │                         ▼
         │                     P12 Voice-guided AR garden mapping and solar context
         │                         │
         │                         ▼
...
Future research (outside release phases):
Automated multi-capture garden reconstruction
```

P3 and P4 may run in parallel after P2. P9 and P10 may run in parallel after foundation validation.
P11 follows the current P10 plant-identification work so it can reuse the evaluated provider boundary
while replacing the incomplete plant UX and data model as one vertical product increment. P12
retains its independent field-mapping research gate. P13 and P14 require validated structured garden
data but do not need to block one another. Automated reconstruction can enter the release graph only
through a future owner-approved ADR and a newly numbered phase.

### 7.1 Roadmap Consolidation Decision

The earlier Garden Scan phase overlapped Phase 12 in capture guidance, point/line/polygon marking,
measurement, alignment, synchronization, proposal review, and web correction. Those delivery items
now have one owner: Phase 12. The only non-duplicate idea—automatic multi-capture reconstruction—is
kept as unnumbered research because no evidence yet shows lower total user effort after correction.

Conversational assistance and 3D/Time Machine do not duplicate Phases 11 or 12: they consume the
plant, journal, map, georeference, and capture data produced there, but add separate interaction,
safety, rendering, accessibility, and value-validation problems. They therefore remain separate
delivery phases, renumbered to Phase 13 and Phase 14. Combining them would create no shared user
outcome or release gate and would couple unrelated AI-assistant and visualization risks.
...
| G10 Plant intelligence and journal quality | Actual/candidate semantics, licensed plant profiles, search and filters, repeat-photo journal, safe health review, accessible image-led native/web UX, offline recovery, sharing isolation, and provider failure pass | Plant intelligence and journal rollout   |
| G11 Field-mapping quality                  | Voice/touch AR mapping, georeference, long boundaries, synchronization, and solar context meet accuracy, safety, accessibility, privacy, and correction-effort thresholds                                             | Field-mapping rollout                    |
| G12 Assistant quality                      | Selected conversational tasks meet authorization, factuality, confirmation, safety, bilingual quality, deletion, latency, and cost thresholds                                                                         | Assistant rollout                        |
| G13 3D and Time Machine value              | 3D, Time Machine, and optional AR visualization each show measurable value and pass identity, accuracy, accessibility, performance, and non-prediction gates                                                          | Independent visualization rollout        |
...
Implementation status (July 24, 2026): P6-DATA-01, P6-PLAT-01, P6-API-01, P6-ASYNC-01, P6-WORKER-01,
P6-WORKER-02, and P6-IOS-01 are implemented. P6-WORKER-01 replaces the prior direct Cloud Tasks → API placeholder
...
metadata/outcomes, signed-access gating, a malicious-fixture suite, and a production Dockerfile. Neither
worker is yet rolled out to `verdery-dev`; Cloud SQL IAM wiring, an always-CPU Cloud Run configuration
for the interval relay, a selected malware provider, and (new with P6-WORKER-02) a derived-bucket
write IAM grant remain explicit deployment dependencies. PDF validation fails retryably while that
scanner is unavailable rather than recording a false clean result. P6-WORKER-02 adds real thumbnail/
screen-preview/high-resolution/tile-pyramid derivative generation via `sharp` (now a real production
dependency), a direct worker-owned GCS write to the derived bucket, and idempotent registration of
each produced derivative as its own new `media_record` row, chained automatically off a successful
raster-eligible validation via a second outbox event and job kind. PDF page-preview rendering remains
out of scope, needing an unevaluated rasterizer dependency (`poppler`/`pdftoppm` or `pdf.js`+WASM).
P6-IOS-01 adds a new `CoreMediaTransfer` module: local-file-first durability, a real Cloud Storage
resumable-upload client and background-capable `URLSession` transport, registration/upload/complete/
...
found the already-shipped manual dimension entry, and Phase 12's on-site AR marking, are both a
...
| ID           | Work package                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Primary               | Dependencies                               | Completion evidence                                                         |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------- | ------------------------------------------ | --------------------------------------------------------------------------- |
| P10-DATA-01  | Add capture session, capability class, media references, quality observations, calibration, processing state, and cancellation/recovery fields — retained as Phase 12 AR groundwork, not for photo/video object capture                                                                                                                                                                                                                                      | WS-DATA               | P6 media                                   | Migration and lifecycle tests                                               |
...
It returns to this table for Phase 11 enrichment. Automated reconstruction research does not
authorize production asynchronous processing; a future delivery phase may reintroduce it only after
`RECON-DECISION-01`.
...
- Manual plant entry (unaffected) and manual garden-object entry (unaffected; AR remains Phase 12's
...
## 20. Phase 11 — Plant Intelligence, Candidates, and Visual Journal

### 20.1 Outcome

Plant management becomes a complete image-led product experience instead of a basic inventory form.
Every add flow distinguishes an actual plant from a candidate. Both kinds reuse a licensed,
source-attributed plant knowledge profile, while only actual plants own care history and a repeated
visual journal. Users can search and filter the complete plant library, evaluate a candidate for a
specific garden location, convert it after planting, photograph the same actual plant repeatedly,
compare progress, and review uncertain health suggestions on native Apple and web clients.

The phase is a vertical product increment. Database and API completion without the corresponding
plant-list, add, search, filter, detail, journal, comparison, and recovery UI does not satisfy G10.

### 20.2 Product and Provider Gate

Before production enablement:

- Approve the actual-versus-candidate terminology, conversion behavior, tracking granularity, and
  minimum add fields through native and responsive-web usability review.
- Select or contract source adapters by knowledge class; confirm commercial reuse, caching,
  attribution, refresh, deletion, and media-transformation rights.
- Approve the initial commercial-media license allowlist and attribution presentation.
- Approve the source-priority, conflict, geographic-scope, cultivar, regulatory, toxicity,
  edibility, and horticultural-review policy.
- Evaluate health suggestions on representative real observations, including false positives,
  missing views, provider refusal, and no-diagnosis outcomes.
- Record storage, enrichment, media-derivative, AI-analysis, and refresh budgets.

Until a source or analysis capability passes its gate, the product shows partial knowledge and
manual observation behavior rather than fabricated facts or placeholder results.

### 20.3 Work Packages

| ID            | Work package                                                                                                                                                                                                                                                                                                                                                         | Primary                     | Dependencies                            | Completion evidence                                                         |
| ------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------- | --------------------------------------- | --------------------------------------------------------------------------- |
| P11-PROD-01   | Freeze actual plant, candidate, individual/row/group, conversion, unknown identity, profile completeness, observation, phenology, symptom, and health-suggestion semantics                                                                                                                                                                                           | WS-PROD, WS-PLANT, WS-GUIDE | P10 identification behavior             | Approved glossary, state diagrams, and safety policy                        |
| P11-DESIGN-01 | Design and test an image-led, icon-led, accessible information architecture for plant library, actual/candidate views, add flow, catalog search, filters, photo cards, detail hero, icon fact strip, suitability, journal capture, timeline, before/after, measurement history, health review, and all empty/loading/partial/offline/error states                    | WS-DESIGN                   | P11-PROD-01                             | Responsive web and native prototype findings resolved or tracked            |
| P11-DATA-01   | Add explicit actual-plant and candidate records, proposed placement, suitability versions, alternatives, purchase facts, idempotent candidate conversion, stable plant identity, and preserved conversion history                                                                                                                                                    | WS-DATA, WS-PLANT           | P4 plant schema                         | Reviewed migrations, invariants, and rollback evidence                      |
| P11-DATA-02   | Add canonical taxon/name/external-ID crosswalks, source records, append-oriented fact and distribution assertions, licensed media metadata, profile versions, geographic/cultivar scope, freshness, conflict, confidence, and review state                                                                                                                           | WS-DATA, WS-PLANT           | P11-PROD-01                             | Data dictionary, provenance, conflict, and rebuild tests                    |
| P11-PROV-01   | Score and select taxonomy, US distribution, occurrence, horticultural-care, phenology, hardiness/climate, soil, regulatory, identification, and image sources; document API/bulk/partnership use and per-asset licenses instead of scraping                                                                                                                          | WS-PLANT, WS-SEC, WS-PROD   | Product and Provider Gate               | Provider decisions, legal/privacy inventory, source runbooks, and fixtures  |
| P11-ASYNC-01  | Implement deduplicated taxon enrichment, identity resolution, source adapters, normalization, license rejection, media classification, cited profile materialization, refresh, partial-result recovery, retry, quota, and kill-switch behavior                                                                                                                       | WS-BE, WS-PLANT, WS-PLAT    | P11-DATA-02, P11-PROV-01                | Job lifecycle, idempotency, provider-failure, and cost tests                |
| P11-SUIT-01   | Implement versioned garden-specific candidate suitability with matches, cautions, blockers, unknowns, assumptions, evidence, and recalculation when garden context or source facts change                                                                                                                                                                            | WS-BE, WS-GUIDE, WS-PLANT   | P11-DATA-01..02, P7 context             | Reviewed rule fixtures and explanation tests                                |
| P11-API-01    | Add contract-first catalog, actual plant, candidate, conversion, enrichment-status, suitability, journal, comparison, media-association, and health-review APIs with pagination, revisions, authorization, idempotency, and sync semantics                                                                                                                           | WS-CONTRACT, WS-BE          | P11-DATA-01..02                         | OpenAPI generation, HTTP integration, and compatibility tests               |
| P11-SEARCH-01 | Extend search projections for scientific names, synonyms, localized common names, cultivars, user names, and match explanation; add server filters for kind, granularity, location, identity, lifecycle, phenology, health, native/regulatory state, life cycle, care requirements, compatibility, bloom/fruit period, completeness, enrichment, and journal recency | WS-DATA, WS-BE, WS-PLANT    | P11-DATA-01..02                         | Relevance, facet, authorization, localization, and query-plan tests         |
| P11-MEDIA-01  | Extend observations into a visual journal with purpose-labeled multi-photo capture, typed measurements, phenology, symptoms, context snapshots, resumable upload, verification, safe EXIF handling, responsive derivatives, duplicate detection, comparison sets, and generated time-lapse derivatives                                                               | WS-MEDIA, WS-BE, WS-PLANT   | P6 media, P11-API-01                    | Upload/recovery, processing, retention, deletion, export, and cost evidence |
| P11-HEALTH-01 | Implement additional-view requests and uncertain health suggestions against a user-selected plant and its history; store model/version/confidence/evidence/disposition while preventing model writes to toxicity, edibility, pesticide, treatment, or regulatory facts                                                                                               | WS-GUIDE, WS-BE, WS-SEC     | P10-PLANT-02, P11-MEDIA-01              | Safety evaluation, false-positive review, refusal, and kill-switch tests    |
| P11-WEB-01    | Replace the basic plant UI with responsive image cards/rows, actual/candidate segmentation and combined view, catalog/add/unknown flows, complete search/filter state, detail hero and icons, cited knowledge, suitability, journal upload, timeline, comparison, measurement history, health review, bulk actions, keyboard behavior, and recoverable drafts        | WS-WEB, WS-DESIGN           | P11-API-01, P11-SEARCH-01, P11-MEDIA-01 | Component, accessibility, responsive-browser, and real E2E tests            |
| P11-IOS-01    | Deliver equivalent native plant-library, add, candidate, search/filter, profile, suitability, journal, comparison, and health-review meaning with camera-first guided shot purposes, previous-image framing overlay, durable offline observation drafts, background upload, Dynamic Type, and VoiceOver                                                              | WS-IOS, WS-DESIGN           | P11-API-01, P11-SEARCH-01, P11-MEDIA-01 | View-model, GRDB migration, offline/restart, real-device, and E2E tests     |
| P11-SHARE-01  | Allow explicit publication of selected progress observations, before/after media, and time-lapse derivatives while excluding candidates, internal suitability notes, health suggestions, precise EXIF/location, rejected analysis, and unpublished media                                                                                                             | WS-SHARE, WS-MEDIA, WS-SEC  | P9C publication, P11-MEDIA-01           | Entitlement, withdrawal, export, and cross-client isolation tests           |
| P11-OBS-01    | Add privacy-safe metrics for search success, filters, add abandonment, actual/candidate creation, enrichment coverage, license rejection, suitability review, conversion, journal capture/recovery, comparison use, health disposition, processing latency, and storage growth                                                                                       | WS-OPS, WS-PLANT            | P11 clients and workers                 | Event catalog, dashboards, consent tests, and alert candidates              |
| P11-QA-01     | Execute the cross-platform domain, provider, search/filter, sync, offline, media, AI safety, accessibility, localization, performance, deletion/export, sharing, failure, and cost matrix                                                                                                                                                                            | WS-QA                       | All P11 implementation                  | G10 evidence report                                                         |

### 20.4 Required UI States and Interactions

The UI evidence must cover more than happy-path screenshots:

- Actual, candidate, combined, unknown, row, group, converted, archived, and removed states.
- Image-card and compact-row layouts with large representative images and useful fallback artwork.
- Icon controls with accessible names, adjacent labels or tooltips where meaning is ambiguous,
  selected/disabled states, keyboard focus, and non-color status cues.
- Search by accepted name, common name, synonym, cultivar, and user-defined name with visible match
  explanation.
- Filter chips, full filter panel, active-filter summary, clear-all, zero-results recovery, saved
  native state, and shareable web query state where privacy permits.
- Add from catalog, camera, existing image, and unknown entry, with the actual/candidate decision
  first or inherited from an unambiguous entry point.
- Candidate proposed placement, suitability evidence, alternatives, and explicit conversion.
- Plant detail hero, icon facts, cited care, distribution, source freshness, conflicts, and partial
  data.
- Journal capture purposes, upload progress, offline queue, retry, duplicate warning, timeline,
  before/after, matched overlay, measurement history, stage/symptom filters, and time-lapse.
- Health-analysis waiting, insufficient evidence, additional-photo request, multiple possibilities,
  low confidence, provider unavailable, rejected suggestion, and manual-only paths.

Images and icons receive greater visual priority, but status, action, attribution, uncertainty, and
safety meaning remain available as accessible text.

### 20.5 Exit Criteria

- Every add surface distinguishes an actual plant from a candidate without counting planned plants
  as present.
- Candidate conversion is explicit, idempotent, revision-aware, and history-preserving.
- Plant creation succeeds during source outages; enrichment is asynchronous, deduplicated,
  provenance-preserving, license-filtered, refreshable, and visibly partial when incomplete.
- Search and every documented filter work against authorized server results and have complete native
  and web interaction states.
- Plant library, add, detail, suitability, journal, comparison, and health-review workflows have the
  same product meaning on native and web.
- Actual plants retain stable identity across repeated photo observations, map movement, renaming,
  re-identification, synchronization, and publication.
- Native journal capture survives offline operation, process termination, and interrupted upload
  without duplicating the observation.
- Media licenses, attribution, EXIF privacy, derivatives, retention, deletion, export, and client
  entitlements pass.
- Health output remains an uncertain suggestion, can request more evidence, requires review, and
  cannot bypass recommendation safety policy.
- Large images and icons lead the presentation while accessibility, localization, keyboard,
  VoiceOver, Dynamic Type, contrast, reduced motion, and non-color meaning pass.
- G10 is approved.

### 20.6 Source Traceability

The product requirements are defined in
[technical-specification.md](technical-specification.md), sections "FR-19: Plant Records",
"FR-20: Plant Addition", "FR-23: Monitoring and Observations", "FR-35: Plant Knowledge and
Candidate Suitability", and "FR-36: Plant Library, Search, and Visual Journal Experience". Domain,
provider, media, API, synchronization, safety, sharing, client, search, and completion behavior is
defined in
[architecture/plant-intelligence-and-visual-journal.md](architecture/plant-intelligence-and-visual-journal.md).

## 21. Phase 12 — Voice-Guided AR Garden Mapping and Solar Context

### 21.1 Outcome

Supported Apple devices provide a voice-guided, user-controlled field-mapping flow. A user names the
semantic object, explicitly marks visible real-world points, captures checkpointed long boundaries,
hears and sees estimated dimensions and area, and adds lot, structure, deck, fence, path, zone, bed,
tree, plant, and annotation geometry without typed input. The session initializes an optional
GPS-based location, confirmed address, and true-north orientation, aligns results to the
garden-local map, persists every committed point, and synchronizes accepted editable objects to web.
Supported depth/LiDAR improves measurement and obstacle-height evidence but is not required.
Georeferenced gardens can explore versioned solar and shadow context on iOS and web. Unsupported or
permission-denied devices retain complete touch, manual, plan, and local-only flows.

### 21.2 Research Gate

Approve device capability tiers and use-case-specific error thresholds using physical reference
measurements across representative outdoor surfaces, distances including checkpointed lot edges,
light, weather, motion, occlusion, magnetic interference, and garden layouts. Evaluate outdoor
speech recognition across supported locales, noise, wind, and connectivity conditions; reverse
geocoding and geographic imagery licensing and availability; and solar/shadow model error against
known locations, dates, obstacle dimensions, and heights. Do not use one global accuracy claim.

### 21.3 Work Packages

| ID               | Work package                                                                                                                                                                                                                                                                                                                                                          | Primary                   | Dependencies                                                           | Completion evidence                                          |
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------ |
| P12-PROD-01      | Approve the initial semantic object catalog, fixed voice-command grammar, supported locales, confirmation rules, spoken-feedback behavior, touch/accessibility parity, measurement language, and field-session state machine                                                                                                                                          | WS-PROD, WS-DESIGN        | Phase 3 object model                                                   | Versioned command and interaction specification              |
| P12-RESEARCH-01  | Benchmark ARKit tracking, relocalization, checkpoint distance, accumulated error, heading interference, depth, and LiDAR across launch device tiers and representative long-boundary conditions                                                                                                                                                                       | WS-IOS, WS-CV             | Research Gate                                                          | Device/use-case capability matrix                            |
| P12-GEO-01       | Add revisioned georeference authoring: GPS initialization, accuracy/time/provenance, reverse-geocoded address with confirmation/edit/omit, true-north orientation, manual/map-pin/control-point fallback, geographic imagery context, privacy enforcement, API contracts, offline representation, and iOS/web settings                                                | WS-DATA, WS-BE, WS-IOS    | P3 geospatial model, P5 sync                                           | Migration, contract, privacy, and cross-client authoring E2E |
| P12-VOICE-01     | Implement contextual microphone permission, visible recording state, push-to-talk/session boundary, localized bounded speech recognition, deterministic command parsing, confidence confirmation, optional spoken results, no-raw-audio default, and complete touch/VoiceOver fallback                                                                                | WS-IOS, WS-DESIGN         | P12-PROD-01                                                            | Outdoor/noisy-condition command and accessibility tests      |
| P12-IOS-01       | Implement capability detection and the camera-first capture state machine: semantic object indicator, reticle and explicit point commit, line/polygon/height marking, live dimensions/area/uncertainty, undo, close/cancel, pause/resume, per-point durable partial save, and quality guidance                                                                        | WS-IOS                    | P10-DATA-01, P12-PROD-01; AVFoundation coordinator removed by ADR-0015 | Real-device interaction and interruption tests               |
| P12-MEASURE-01   | Convert AR observations to application-owned SI measurements, transforms, height facts, quality, uncertainty, and calibration records; calculate and report segment length, cumulative length, perimeter, and polygon area                                                                                                                                            | WS-IOS, WS-MAP            | P12-IOS-01                                                             | Reference-measurement and area fixtures                      |
| P12-ALIGN-01     | Implement first-segment local origin, accepted garden true-north relationship, two-control-point alignment, explicit later realignment, residual error, versioned alignment records, and the rule that device heading cannot silently replace garden orientation                                                                                                      | WS-MAP, WS-IOS, WS-BE     | P12-GEO-01, P12-MEASURE-01                                             | Heading, alignment, and disagreement tests                   |
| P12-FIELD-MAP-01 | Implement voice-or-touch creation flows for lot boundary, structure/house, deck/veranda, fence, gate, path, zone, bed, water feature, utility exclusion, tree, plant, and annotation, using the correct point/line/polygon semantics and a shared finish/review flow                                                                                                  | WS-IOS, WS-MAP, WS-DESIGN | P12-VOICE-01, P12-MEASURE-01                                           | Required-object field journey matrix                         |
| P12-SEGMENT-01   | Support long-object and long-lot-boundary capture through recoverable checkpoints, shared control points, quality-triggered pause, GPS/geographic-context constraint, accumulated-error warning, session stop/resume, later realignment, and independent segment correction; explicitly evaluate distances on the order of 200 m                                      | WS-IOS, WS-DESIGN, WS-MAP | P12-ALIGN-01                                                           | Long fence and lot-boundary field tests                      |
| P12-LIDAR-01     | Add optional depth and scene observations behind capability checks; evaluate plane, edge, obstacle, scale, and obstacle-height improvement without changing core availability                                                                                                                                                                                         | WS-IOS, WS-CV             | P12-RESEARCH-01                                                        | Quality delta versus non-LiDAR baseline                      |
| P12-SUN-01       | Implement versioned solar-position and shadow analysis from location, true north, selected time/date, accepted obstacle footprints, height facts, and supported terrain; distinguish calculated shadows from imagery shadows; expose missing inputs, uncertainty, representative seasons, time control, and direct-sun duration where evidence supports it            | WS-MAP, WS-GUIDE, WS-WEB  | P12-GEO-01, P12-MEASURE-01                                             | Known-site solar/shadow fixtures and cross-client parity     |
| P12-PROPOSAL-01  | Convert finished AR/depth results into ordinary editable map proposals with provenance, uncertainty, stale-revision protection, explicit acceptance, atomic local projection/outbox writes, resumable P5 synchronization, and source/status visibility on web                                                                                                         | WS-BE, WS-MAP, WS-IOS     | P12-ALIGN-01, P12-FIELD-MAP-01, P5 sync                                | Offline-to-web proposal acceptance E2E                       |
| P12-WEB-01       | Present synchronized field-created objects in the web editor with ordinary geometry editing, source/author/time/accuracy inspection, north and geographic context, incomplete-proposal status where authorized, and solar/shadow time and season exploration                                                                                                          | WS-WEB, WS-DESIGN         | P12-GEO-01, P12-PROPOSAL-01, P12-SUN-01                                | Responsive web editing and parity E2E                        |
| P12-SAFE-01      | Implement continuous camera/microphone state, tracking-degradation pause, obstacle/private-property guidance, no backward-walking instruction, estimate/non-survey disclaimers, exact-location publication controls, and safe recovery without losing completed points                                                                                                | WS-DESIGN, WS-SEC         | P12-IOS-01, P12-VOICE-01, P12-GEO-01                                   | Safety, privacy, and observed field study                    |
| P12-QA-01        | Test permission denial/regrant, noisy or ambiguous speech, interruption and process death, offline continuation, poor tracking, magnetic interference, 200 m checkpoint capture, relocalization failure, unsupported devices, address/geocoder/imagery absence, alignment conflicts, shadow missing inputs, cross-client sync, and complete non-voice/manual fallback | WS-QA, WS-SEC             | All P12 implementation                                                 | G11 field-mapping evidence                                   |

### 21.4 Exit Criteria

- Supported use cases meet approved error and completion thresholds; unsupported cases are disabled or labeled experimental.
- A user can create the required semantic object set through voice plus explicit camera-point
  selection without typing, and can complete the same flow without microphone access.
- A long boundary can pause, checkpoint, resume, realign, expose accumulated uncertainty, and
  survive process termination without losing committed points.
- A completed scaled lot reports perimeter, area, source, and uncertainty visually and through
  optional spoken feedback.
- Location initialization can produce a user-confirmed address and true-north orientation, while
  location denial, missing imagery, and missing reverse geocoding retain full local mapping.
- Accepted field objects synchronize through the existing offline protocol and appear as ordinary
  editable objects in web with source and uncertainty.
- Calculated shadows are distinct from imagery shadows, expose their input completeness, and provide
  consistent time/season behavior on iOS and web.
...
- G11 is approved independently for each enabled capability tier.

### 21.5 Source Traceability

Voice-guided semantic capture, explicit physical-point selection, measurement, long-boundary
checkpointing, location/address/north authoring, alignment, solar and shadow context,
synchronization, capability tiers, safety, and manual fallback are specified in the product and
capture designs. [Source: technical-specification.md, sections "FR-10: Satellite or Map-Image
Start", "FR-13: On-Site AR Marking", "FR-14: AR-to-Map Alignment", "FR-15: GPS and Geographic
Positioning", "FR-15A: Solar and Shadow Context", and "FR-17: Plot Area Estimate";
architecture/garden-capture-and-scan.md, sections "5. Capture Capability Tiers", "7. Safety UX
Requirements", "9. AR Measurement Flow", "9.2 Long Boundary Capture", "9.3 Geographic
Initialization", and "9.5 Acceptance and Synchronization";
architecture/data-and-geospatial-design.md, section "9. Georeferencing";
architecture/map-rendering-and-editing.md, sections "3.2 Geographic Space" and "16.2 Geographic and
Solar Context"]

## 22. Future Research — Automated Garden Reconstruction

### 22.1 Status and Boundary

Automated reconstruction is not a committed release phase. Phase 12 owns guided field capture,
checkpoints, alignment, validation, proposal acceptance, synchronization, and web editing. This
research track must not duplicate those capabilities or reintroduce the rejected casual
photo/video-to-garden-object workflow under a different name.

The only retained question is whether a deliberately guided, multi-capture reconstruction system can
reduce total user effort beyond Phase 12 while meeting accuracy, correction-effort, privacy,
licensing, reliability, and unit-economics thresholds. No production pipeline, client surface,
provider contract, raw-media collection program, or release forecast is authorized by this section.

### 22.2 Research Tracks

| ID                | Research question                                                                                                                                      | Evidence required                                                                 |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------- |
| RECON-SCOPE-01    | Identify use cases where Phase 12 remains materially too slow or incomplete and define a measurable user-effort advantage for automation               | Phase 12 field evidence and approved proceed/stop thresholds                      |
| RECON-TECH-01     | Benchmark owned and specialist multi-capture reconstruction approaches, compute requirements, reproducibility, licensing, retention, and failure modes | Disposable benchmark report; no production integration                            |
| RECON-QUALITY-01  | Compare object precision/recall, geometry error, correction effort, completion rate, and failure transparency against the Phase 12 baseline            | Consented representative evaluation protocol and benchmark results                |
| RECON-PRIVACY-01  | Evaluate neighboring-property capture, raw-media minimization, provider training terms, deletion feasibility, and derivative sensitivity               | Privacy impact assessment and acceptable provider/legal posture                   |
| RECON-ECON-01     | Estimate CPU/GPU, storage, network, support, review, and failed-run cost per useful accepted object                                                    | Unit-economics model with stop thresholds                                         |
| RECON-DECISION-01 | Decide whether evidence justifies a new numbered delivery phase                                                                                        | New owner-approved ADR that explicitly reconciles or supersedes ADR-0015 in scope |

### 22.3 Promotion Criteria

Automated reconstruction may return to the release roadmap only when:

- Phase 12 field evidence identifies a material unsolved mapping problem.
- A benchmark demonstrates lower total user effort after correction, not only higher automation.
- Accuracy, privacy, provider rights, reproducibility, failure recovery, and unit economics meet
  approved thresholds.
- A new ADR defines the capture modality and distinguishes it from rejected casual photo/video
  garden-object recognition.
- The implementation receives a new phase number, work packages, estimate, and independent release
  gate at that time.

Failure to satisfy any criterion closes or pauses the research without affecting manual, plan, or
Phase 12 mapping.

### 22.4 Source Traceability

The research-only boundary reconciles the accepted decision to prefer manual dimensions and Phase 12
AR marking over casual photo/video object recognition with the architecture's retained possibility
of specialized future reconstruction. [Source:
architecture/decisions/ADR-0015-phase10-redirect-plants-over-photo-capture.md, sections "Decision"
and "Rejected Alternatives"; technical-specification.md, section "FR-16: Automated Garden
Reconstruction Research"; architecture/garden-capture-and-scan.md, section "Future Research:
Automated Reconstruction"]

## 23. Phase 13 — Constrained Conversational Assistance
...
## 24. Phase 14 — 3D Garden and Time Machine
...
| P14-AR-01   | Evaluate optional AR visualization only after 3D value and AR capability evidence; reuse accepted object identity and transforms                    | WS-IOS                 | P14-IOS-01, P12 AR   | Separate proceed/stop gate                 |
| P14-QA-01   | Test identity consistency, geometry alignment, lifecycle states, unsupported data, visual accessibility, performance, and misleading-precision risk | WS-QA                  | All enabled P14 work | G13 3D/Time evidence                       |
...
- G13 is approved independently for 3D, Time Machine, and any AR visualization.

### 24.5 Source Traceability
...
## 25. Continuous Work Across All Phases
...
### 25.3 Observability and Operations
...
### 25.4 Documentation and Architecture Governance
...
## 26. Surface Delivery Matrix

### 26.1 Foundation Surfaces
...
| Automated reconstruction            | Research only; no committed surface   | None              | New ADR and newly numbered delivery phase required before implementation         |
...
| Catalog search and complete filters         | Required with restorable native state             | Required with shareable query state where privacy permits   | Not available                                          |
| Candidate suitability and conversion        | Required                                          | Required; preferred for detailed comparison                 | Never exposed unless future publication policy changes |
| Source-attributed plant profile             | Required                                          | Required                                                    | Only selected safe published facts                     |
| Repeated visual observations                | Camera-first, offline-capable, background upload  | Camera/file input and recoverable upload                    | Only explicitly published observations                 |
| Timeline and before/after comparison        | Required                                          | Required                                                    | Required for entitled published media                  |
| Measurement history and time-lapse          | Required                                          | Required                                                    | Only explicitly published derivative                   |
| Health suggestions and additional-view flow | Required, uncertainty-first                       | Required, uncertainty-first                                 | Not exposed                                            |
| Icon and image accessibility                | VoiceOver, Dynamic Type, non-color status         | Keyboard, screen reader, alternative text, non-color status | Same portal accessibility standard                     |

This surface matrix is required completion evidence for P11. A backend-only catalog, candidate,
enrichment, journal, or health-analysis implementation does not satisfy G10. [Source:
technical-specification.md, sections "FR-35: Plant Knowledge and Candidate Suitability" and "FR-36:
Plant Library, Search, and Visual Journal Experience";
architecture/plant-intelligence-and-visual-journal.md, sections "11. API Surface", "12. Search and
Filters", and "13. Visual and Interaction Design"]

## 27. Functional Requirements Traceability
...
| FR-9 Progressive Map Creation                              | P3, P6, P12     | Blank, imagery, plan, manual, capture, and proposal paths (redirected from P10 to P12 AR by ADR-0015)                              |
| FR-10 Satellite or Map-Image Start                         | P0, P3, P12     | Provider/license decision, attribution, tracing, GPS centering, address confirmation, imagery-shadow distinction, outage fallback  |
...
| FR-13 On-Site AR Marking                                   | P12             | Device benchmark and real-world marking tests                                                                                      |
| FR-14 AR-to-Map Alignment                                  | P12             | Control-point, residual, relocalization, realignment tests                                                                         |
| FR-15 GPS and Geographic Positioning                       | P3, P12         | Optional location context and precision-warning tests                                                                              |
| FR-15A Solar and Shadow Context                            | P12             | Solar-model fixtures, obstacle/height completeness, time/season controls, uncertainty, and iOS/web parity                          |
| FR-16 Automated Garden Reconstruction Research             | Future research | Phase 12 comparison, disposable benchmark, privacy/licensing assessment, unit economics, and a proceed/stop ADR                    |
| FR-17 Plot Area Estimate                                   | P3, P12         | Scaled/approximate area, spoken and visual completion report, and uncertainty tests                                                |
| FR-18 Map Provenance and Accuracy                          | P3, P6, P12     | Provenance, accuracy state, verification, and revision fixtures                                                                    |
| FR-19 Plant Records                                        | P4, P6, P11     | Actual/candidate distinction, plant details, photos, placement, lifecycle, conversion, and history                                 |
| FR-20 Plant Addition                                       | P4, P6, P10–P11 | Manual, unknown, individual/group, photo identification, actual/candidate choice, catalog, and async enrichment                    |
...
| FR-23 Monitoring and Observations                          | P4, P6, P10–P11 | Append history, repeated purpose-labeled media, measurements, offline recovery, comparison, and uncertain analysis suggestions     |
...
| FR-28 Conversational Assistance                            | P13             | Authorized retrieval, confirmation, safety, and bilingual evaluation                                                               |
...
| FR-33 Web Application                                      | P1–P14          | Operational and client route matrices, responsive plant and field-map UX, assistant, 3D, accessibility, continuity, and E2E        |
...
| FR-35 Plant Knowledge and Candidate Suitability            | P11             | Source/provider contracts, assertion provenance, licensed media, enrichment, suitability, conversion, and partial-profile evidence |
| FR-36 Plant Library, Search, and Visual Journal Experience | P11             | Native/web list, add, search, filters, detail, journal, comparison, health-review, image/icon accessibility, and E2E evidence      |
...
| P11 Plant intelligence and journal |              14–22 weeks | source contracts and licensing, taxonomy/cultivar reconciliation, complete native/web UX, offline media, search/filter breadth, and health-safety evaluation | Provider/legal, data/API, media, web, and native tracks parallelize after semantics and contracts |
| P12 voice-guided AR field mapping  |              18–30 weeks | outdoor AR accuracy, long boundaries, speech, georeference/address providers, solar-model scope, web parity, and device diversity                            | Follows its independent research gate; split rollout by proven capability tier                    |
...
critical path. That estimate is retained as planning history, not a statement about the completed
implementation sequence. P9 is now delivered and P10 is the active phase. P11 is the next integrated
product phase requested by the owner; its data, API, provider, media, native, and web work must not
be split into separately declared product completions. P12–P14 remain independent investment
decisions and may be reordered, reduced, or stopped after their evidence gates. Automated
reconstruction has no delivery estimate because it is not a committed phase.

### 28.3 Reforecast Rules
...
## 29. Execution Sequence and Current Position

This is a dependency-aware sequence, not a fixed sprint commitment. Status is based on repository
evidence on July 28, 2026; a completed increment does not by itself approve a release gate.
...
|         3 | Firebase native/web authentication and profile provisioning                                          | Implementation complete; G2 pending                                           | Same user signs in on native and web                              |
|         4 | Garden create/list/open with revisions and authorization                                             | Implementation complete; G2 pending                                           | G2 first-garden vertical slice                                    |
|         5 | Coordinate spaces, garden object schema, shared geometry fixtures                                    | Implementation complete; G3 pending                                           | Same fixtures pass in backend, Swift, and TypeScript              |
|         6 | Basic point/line/polygon editor on web and native                                                    | Implementation complete; G3 pending                                           | Lot, house, fence, and bed created and reopened                   |
|         7 | Full required object tools, layers, undo/redo, scale/provenance                                      | Implementation complete; G3 pending                                           | G3 representative garden                                          |
|         8 | Plants/groups, placement, observations, and history                                                  | Implementation complete; G4 pending                                           | Plant selected from map opens shared record                       |
|         9 | Manual tasks, lifecycle, localization, accessibility, care analytics                                 | Implementation complete; G4 pending                                           | G4 care-data workflow                                             |
|        10 | Sync push/pull, local outbox, initial snapshot                                                       | Implementation complete; G5 pending                                           | Offline create survives restart and syncs                         |
|        11 | Conflicts, tombstones, revocation, full resync, upgrades                                             | Implementation complete; G5 pending                                           | G5 fault-injected convergence                                     |
|        12 | Direct photo upload and verification foundation                                                      | Implementation complete; G6 pending                                           | Interrupted upload resumes without duplicate record               |
|        13 | Real photo-based plant identification and condition adapter plus property-plan OCR                   | P10 in progress                                                               | User sees and confirms a real provider suggestion on both clients |
|        14 | Actual/candidate semantics, source-attributed plant knowledge, visual journal, and complete plant UX | Planned as P11 after P10                                                      | G10 cross-platform plant-library and journal journey              |

Later increments delivered the rest of P6, the P7 care loop, P8 hardening, and all four P9
subphases. Their implementation and remaining gate boundaries are summarized in Section 2.4 and
recorded in detail in `tasks/todo.md`. Every future increment must still end in a demonstrable
integrated state; incomplete hidden layers do not count as completed product outcomes.

## 30. Decision Register

### 30.1 Remaining Decisions Required to Close G0
...
| Foundation surface parity | Determines parallel client scope                          | Open in product spec                                                | Approve Section 26                                                                               | WS-PROD                | Before P3/P4 backlog       |
...
| Providers                 | Affects licensing, privacy, quality, cost, and UX         | Implementation-time; weather decided July 26, 2026 (section 30.1.1) | Score capability-specific adapters; one active provider per environment                          | WS-PROD, WS-BE, WS-SEC | Before dependent phase     |
...
### 30.1.1 Provider Decisions Taken

| Date          | Decision                                                                                                                                                                                                                                                                                                                                                                                | Reasoning                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Consequence                                                                                                                                                                                                                                                       |
| ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| July 26, 2026 | **Weather: Open-Meteo** (paid Standard plan, models pinned to NOAA). Named fallback: NWS / api.weather.gov, not implemented                                                                                                                                                                                                                                                             | **Retention rights, not price.** This system keeps normalized weather rows permanently and append-only. Apple WeatherKit, Tomorrow.io, Google Weather, and WeatherAPI.com were each disqualified by their own terms — no persistent storage, derived-database bans, mandatory deletion TTLs, and an explicit ban on use in a weather application — and OpenWeather by ODbL share-alike. Open-Meteo's CC BY 4.0 permits durable storage and redistribution with attribution, and the licence does not change on the paid plan          | One adapter class plus one registration; every stored row carries CC BY 4.0 and the required attribution link; `models=` is pinned to NOAA sources so no CC-BY-SA source (for example UK Met Office) can enter a record. See external-integrations.md section 5.1 |
| July 26, 2026 | **Transactional email: Resend** (free tier, 3,000 messages/month, no subscription). Named fallbacks: Postmark ($15/month, better established deliverability, upgrade path once organization/client volume justifies the cost) and Amazon SES (near-zero pay-per-use cost at any volume, but bounce/complaint delivery via Amazon SNS rather than a plain webhook) — neither implemented | **Owner's explicit budget constraint**: no monthly subscription at launch. Deliverability and domain-trust reasoning still favors Postmark on paper (independent benchmarks: ~98% inbox placement vs. Resend's less-established sender reputation and no comparable published benchmark), and this is recorded as a real, accepted trade-off, not an oversight — Resend uses the same plain-webhook integration shape as Postmark, so switching later is the registry's normal one-adapter-plus-one-registration swap, not a redesign | One adapter class plus one registration, matching the weather integration's own pattern; revisit once real client-invitation volume or deliverability problems justify the paid alternative                                                                       |

The **plant-content half of `P0-PROV-01` remains open** and is now an explicit P11 product/provider
gate. Photo identification is being resolved in P10. Geocoding and aerial imagery were decided on
August 4, 2026 (section 30.1.1); commercial map/imagery beyond those two remains open.

| August 4, 2026 | **Address geocoding: United States Census Bureau geocoder** (`geocoding.geo.census.gov`, free, no key, public-domain data). Named fallback: a commercial geocoder, not implemented. **Aerial imagery: USGS National Map NAIP** (`imagery.nationalmap.gov`, public domain, no key) | **Licence and coverage, not price.** Both are United States federal public-domain services, which matches ADR-0007's first market and removes the question every commercial alternative raises: Google, Mapbox and HERE each restrict storing or deriving from results, and tracing a lot from imagery IS deriving from it; Nominatim is ODbL share-alike, the same defect that disqualified OpenWeather. Neither service needs an account, which also honours the no-subscription constraint the Resend decision recorded | Two isolated adapters. The query and rejected geocoder candidates remain transient; the accepted coordinate and formatted address persist in the revisioned georeference. Imagery remains a review backdrop; an explicitly enabled bounded export may feed review-only vision proposals, but pixels and unaccepted proposals never become canonical geometry. Coverage ends at the United States border, and both surfaces say so rather than failing silently. |

### 30.2 Decisions Required Before Foundation GA
...
| Capture evaluation dataset absent                                  | Capture design requires representative physical references for Phase 12 and a separately consented dataset before any automated-reconstruction benchmark. [Source: architecture/garden-capture-and-scan.md, sections "9. AR Measurement Flow" and "10.11 Evaluation"]                                                                                | Field-mapping or future reconstruction quality cannot be proven                  |                High for advanced scope | Build Phase 12 reference fixtures now; authorize a reconstruction dataset only after `RECON-SCOPE-01`               |
| Plant-content provider rights and source governance are unresolved | The product now requires durable normalized facts, citations, commercial image use, geographic scope, and refresh behavior, while plant-content provider selection remains open. [Source: technical-specification.md, sections "FR-35: Plant Knowledge and Candidate Suitability" and "FR-36: Plant Library, Search, and Visual Journal Experience"] | P11 could ship incomplete, legally unusable, or misleading profiles              |          Critical before P11 ingestion | Complete P11-PROV-01, approve per-knowledge-class sources and licenses, and keep unsupported fields visibly unknown |
...
| Monetization is open                                               | Monetization is an explicit product question. [Source: technical-specification.md, section "18. Open Product Questions"]                                                                                                                                                                                                                             | Quotas and expensive-feature economics may change                                |                                 Medium | Decide before broad paid AI or other expensive-feature rollout; do not block the foundation core loop               |
...
| Transactional email production readiness is incomplete             | Resend is selected, but production domain authentication, bounce/complaint handling, deliverability evidence, and environment enablement remain owner-operated work. [Source: architecture/external-integrations.md, section "10. Transactional Messaging"]                                                                                          | P9C production invitation and notification E2E cannot pass                       | High before production client delivery | Verify domain, signed webhooks, suppression behavior, monitoring, and fallback criteria                             |

## 32. Contradictions and Tensions

No direct contradiction remains between the approved architecture documents after Draft 0.8 synchronization. The following product-to-implementation tensions still require explicit handling.

### 32.1 Web Recoverable Draft Scope
...
### 32.2 Proposed Product Priority Versus Approved Architecture
...
### 32.3 Guest Storage Versus Guest Product Behavior
...
### 32.4 Implementation Began Before Full G0 Closure

- **Source tension:** G0 is defined as the gate to start implementation, but repository evidence records P1 complete and P2 in progress while several product, provider, research, and commercial decisions remain open. [Source: this document, sections "2.4 Current Implementation Status", "8. Release Gates", and "30.1 Remaining Decisions Required to Close G0"; tasks/todo.md]
...
| Plant facts become misleading through source collapse        | Plant requirements and life cycle vary by geography, cultivar, evidence quality, and time, while multiple sources may disagree. [Source: architecture/plant-intelligence-and-visual-journal.md, sections "4. Data Model", "5. Plant Knowledge Sources", and "10. Suitability Assessment"] | Critical |       High | A profile presents one uncited universal value or silently overwrites a conflict           | Store source assertions separately, materialize cited geographic views, expose unknown/conflict states, and require horticultural review      |
| Plant images violate commercial-use terms                    | Each external image may carry a different license, attribution, transformation, or withdrawal requirement. [Source: architecture/plant-intelligence-and-visual-journal.md, section "7. Life Cycle and Representative Media"]                                                              | Critical |       High | Missing license/creator, noncommercial media in production, or unsupported crop/derivative | Enforce a per-asset allowlist, attribution record, ingestion rejection, reconciliation, and withdrawal path                                   |
| Visual journal storage and analysis costs grow without bound | Repeated originals, responsive derivatives, time-lapses, and health analysis add durable storage and processing. [Source: architecture/plant-intelligence-and-visual-journal.md, sections "8. Visual Plant Journal" and "16. Reliability, Cost, and Provider Failure"]                    |     High |       High | Storage per active garden or derivative queue age grows faster than budget                 | Quotas, lifecycle policy, duplicate detection, derivative budgets, cost telemetry, and degradation that preserves originals and observations  |
| Health suggestion is mistaken for diagnosis                  | Automated plant-health output is uncertain and may be wrong or lack required views. [Source: technical-specification.md, sections "FR-23: Monitoring and Observations" and "13. Accuracy and Safety Policy"]                                                                              | Critical |       High | Users act on low-confidence output or model text bypasses treatment policy                 | Suggestion terminology, visible confidence, additional-view requests, confirmation/disposition, restricted fact fields, and safety evaluation |
| Native and web plant meaning diverges                        | P11 adds actual/candidate state, conversion, source profiles, filters, journal, and health review across both clients. [Source: technical-specification.md, section "FR-36: Plant Library, Search, and Visual Journal Experience"]                                                        |     High |       High | Different filters, conversion outcomes, or observation histories appear across clients     | Contract-first semantics, shared fixtures, surface matrix, cross-client E2E, and G10 prohibition on backend-only completion                   |
...
| Automated reconstruction research does not justify delivery  | No benchmark currently shows lower total user effort than Phase 12 after correction, and unit economics are unknown. [Source: this document, section "22. Future Research — Automated Garden Reconstruction"]                                                                             |     High |       High | Benchmark correction effort or cost fails its stop threshold                               | Keep it outside release phases; require `RECON-SCOPE-01` through `RECON-DECISION-01` and a new ADR before delivery                            |
...
1. Finish P10 without declaring provider-backed identification complete until real native and web
   screens show a suggestion, confidence, and explicit confirmation path.
2. Record the P10 provider spot-check, current image retention/training terms, kill-switch behavior,
   malformed/refused response tests, and property-plan OCR evidence.
3. Complete P11-PROD-01 and P11-DESIGN-01 before schema and API lock: test the actual/candidate
   choice, conversion, image-led plant library, add flow, search, filters, detail, journal capture,
   progress comparison, and health-review states on native and responsive web.
4. Produce P11-PROV-01 source scorecards and legal/privacy inventory for taxonomy, US distribution,
   occurrences, horticultural care, phenology, hardiness/climate, soil, regulation, identification,
   and per-asset images.
5. Approve the initial media-license allowlist, attribution UI, source-conflict policy,
   horticultural review ownership, and refresh policy before importing plant content.
6. Freeze the P11 data dictionary and OpenAPI changes, including offline/synchronization behavior,
   before native and web implementation diverge.
7. Convert every P11 work package into dependency-linked issues that retain vertical completion:
   no server-only issue group may close the phase without both client workflows and G10 evidence.
8. Reforecast P11 after provider rights and the tested design prototype are known; the current range
   is a planning hypothesis, not a commitment.
...
| ---------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| [Architecture index](architecture/README.md)                                                   | P0–P14 plus future research, governance                                          |
| [Native Apple application](architecture/ios-application-design.md)                             | P1–P7, P9–P14                                                                    |
| [Web application](architecture/web-application-design.md)                                      | P1–P14 operational, client, review, assistant, and visualization surfaces        |
| [Map rendering and editing](architecture/map-rendering-and-editing.md)                         | P0, P3, P6, P10, P12, P14, and future reconstruction research                    |
| [Backend modular monolith](architecture/backend-modular-monolith.md)                           | P1–P14                                                                           |
| [REST API and contracts](architecture/api-design.md)                                           | P1–P14                                                                           |
| [Data and geospatial model](architecture/data-and-geospatial-design.md)                        | P1–P14                                                                           |
| [Plant intelligence and visual journal](architecture/plant-intelligence-and-visual-journal.md) | P11 domain, sources, enrichment, media, search, native/web UX, sharing, and G10  |
| [Offline synchronization](architecture/offline-synchronization.md)                             | P1–P6, P9A operational sync and P9C exclusion, P10–P13                           |
| [Identity and authorization](architecture/identity-and-authorization.md)                       | P0–P2, P8–P11 operational, organization, and client capabilities, P13            |
...
| [Media storage and processing](architecture/media-storage-and-processing.md)                   | P6, P8, P9C publication entitlement, P10–P12, and future reconstruction research |
| [Garden capture and reconstruction research](architecture/garden-capture-and-scan.md)          | P0, P6, P10, P12, and future reconstruction research                             |
| [Asynchronous processing](architecture/asynchronous-processing.md)                             | P1, P6–P11 publication/enrichment/notification jobs, P13, and future research    |
| [Recommendations and AI](architecture/recommendations-and-ai.md)                               | P7, P9–P11, P13–P14                                                              |
| [External integrations](architecture/external-integrations.md)                                 | P0, P3, P6–P7, P9C transactional messaging, P10–P11, P13                         |
| [Notifications](architecture/notifications.md)                                                 | P7–P9 team and client notifications                                              |
| [Security and privacy](architecture/security-and-privacy.md)                                   | Every phase; G0–G13                                                              |
...
| [Cost and scaling](architecture/cost-and-scaling.md)                                           | P0–P1, P6–P11 portal/publication/media/enrichment load, P12–P14, future research |
