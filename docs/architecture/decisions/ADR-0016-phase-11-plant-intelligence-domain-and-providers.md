...
| `plant_media_asset`                                                                  | `integrations.plant_media_asset` (built)                                         | BUILT by `1787700000000_plant-taxon-knowledge-profile.sql`. Licensed image metadata with a `discovered`/`rejected`/`ingested` state, storing every licence a source claims — presentation eligibility is an application-layer decision (`integrations/domain/plant-media-asset.ts`), so a licence category's standing can change without a migration. |
...
**This table's `none yet` column is a snapshot of 2026-08-01, not a live index of the schema.**
Several rows above have since been built; one of them sent a later pass to write a duplicate
migration for a table that already existed. Check `services/api/migrations/` before treating any
cell in this column as current.
