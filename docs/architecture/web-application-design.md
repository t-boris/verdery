...
Interaction and composition borrow from Material 3 Expressive rather than from a conventional form-first admin interface: route identity is icon-led, primary actions carry directional motion, portfolio roots use asymmetric dashboard composition, simple field/action pairs share one horizontal baseline, and responsive layouts preserve that relationship until the available width would make the localized labels collide. Garden creation is a command composer rather than a labeled form stack: its contextual icon, name input, and explicit create action share one control, while localized suggestion chips fill the draft without creating a server record. The locally generated `public/images/field-console-botanical.webp` supplies restrained editorial imagery to route headers and record thumbnails; it is decorative, carries no product information, and sits beneath contrast-controlled overlays. Entry and hero motion are brief or slow, decorative only, and disabled by the global reduced-motion rule.
