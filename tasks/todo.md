...
# Phase 11 — Plant Intelligence, Candidates, and Visual Journal, review

P11-QA-01's own scope line (implementation-plan.md section 20.3) is "Execute the cross-platform
domain, provider, search/filter, sync, offline, media, AI safety, accessibility, localization,
performance, deletion/export, sharing, failure, and cost matrix," with "G10 evidence report" as its
named deliverable — mirroring the Phase 10 review's own evidence-table format exactly, per this
phase's own "Work-package sequencing" note above ("P11-QA-01 ... closes the phase, mirroring Phase
8/10's evidence-table review format"). This entry is that closing evidence report.

## Work packages delivered

| Work package  | Delivered                                                                                                                                                                                                                                                                                                                     | Gate that remains                                                                                                                                                                                                                                                                                                                                                                                        |
| ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P11-PROD-01   | Domain freeze (glossary, actual/candidate/conversion/health-suggestion state diagrams, safety policy) — ADR-0016, closed before any schema work, mapping every design-doc concept onto pre-existing Phase 4/7/9D/10 tables rather than forking new ones                                                                       | —                                                                                                                                                                                                                                                                                                                                                                                                        |
| P11-PROV-01   | Every ADR-0016-selected free/public source's real, current access mechanism verified by live request; provider decisions, legal/privacy inventory, source runbooks, and fixtures — documentation/verification, not adapter code (P11-ASYNC-01's own scope)                                                                    | A commercial care-content vendor beyond the free baseline remains an owner-only, undecided, unbuilt gap — ships as visible partial coverage, not a blocker                                                                                                                                                                                                                                               |
| P11-DATA-01   | `plant_candidate`/`candidate_conversion`/`candidate_suitability_assessment` schema, conversion domain logic                                                                                                                                                                                                                   | —                                                                                                                                                                                                                                                                                                                                                                                                        |
| P11-DATA-02   | `plant_fact_assertion` (structured, review-gated) alongside the existing `plant_content_record`; taxon crosswalk/profile schema                                                                                                                                                                                               | —                                                                                                                                                                                                                                                                                                                                                                                                        |
| P11-ASYNC-01  | Real USDA PLANTS adapter, kill-switched; the fetch→persist→invisible-until-reviewed→resolved pipeline; scheduled worker sweep (sixth instance of the established pattern)                                                                                                                                                     | Eight of nine ADR-0016 sources remain adapter-shaped follow-up (same one-adapter-plus-registration shape the shipped one proves); literal Cloud Tasks/Cloud Run Job migration for this sweep and its five siblings; the human-review workflow promoting `awaiting_horticultural_review` to `horticulturally_reviewed`, without which fetched assertions stay invisible to profiles/suitability by design |
| P11-SUIT-01   | Garden-specific suitability engine, three independent rule axes, fixture-reviewed, real-Postgres integration test including the no-identified-taxon path                                                                                                                                                                      | —                                                                                                                                                                                                                                                                                                                                                                                                        |
| P11-API-01    | OpenAPI contract, HTTP transport for candidate CRUD/conversion/suitability/taxon-profile read, over the schema/engine above                                                                                                                                                                                                   | —                                                                                                                                                                                                                                                                                                                                                                                                        |
| P11-SEARCH-01 | Trigram plant/candidate search, `identified` filter, synonym/cultivar/localized-name search-and-dedupe                                                                                                                                                                                                                        | Six more documented, scoped filter dimensions (compatibility, phenology, native/regulatory state, completeness, health, journal recency) plus `location`/"user names" — each real, buildable, none silently expected to land in this pass                                                                                                                                                                |
| P11-MEDIA-01  | Purpose-labeled journal photos, typed measurements, phenology, context snapshot, real Postgres/iOS/web coverage                                                                                                                                                                                                               | Symptoms vocabulary, duplicate detection, generated time-lapse, and comparison sets are ALL genuinely unspecified/infrastructure-absent — no forward-pointing comment in the codebase ever promised them to this pass                                                                                                                                                                                    |
| P11-HEALTH-01 | Evidence summary, alternatives, safety class, four-state disposition — real schema/adapter/route/UI-data wiring                                                                                                                                                                                                               | Additional-view-request UI consumption landed later in P11-WEB-01/IOS-01; real-photo adapter evaluation and Vertex data-retention confirmation remain the same open ADR-0015 owner gates; `PLANT_CONDITION_AI_ENABLED` stays `false` everywhere                                                                                                                                                          |
| P11-WEB-01    | Web candidates surface (list/filter/search/add/detail/suitability/convert), `identified` filter, health-suggestion review UI                                                                                                                                                                                                  | Card-grid layout, URL filter state, bulk actions, full journal-capture UI, taxon-catalog browse remain open, matching this session's own prior deferral notes; no live-browser click-through (no running Postgres/Firebase-backed instance in this sandbox)                                                                                                                                              |
| P11-IOS-01    | iOS `FeatureCandidates` module (list/filter/search/add/detail/suitability/convert), `identified` filter, health-suggestion review UI                                                                                                                                                                                          | Candidate proposed-placement/`alternativeToCandidateId` UI fields, `ObservationPhoto.purpose` modeling, deep-linking from a converted candidate to its new plant remain open; no live-device/simulator click-through in this sandbox                                                                                                                                                                     |
| P11-SHARE-01  | `observation` publication kind end to end (migration→domain→application→persistence→routes→contract→client-portal rendering); a real, pre-existing EXIF/GPS media-safety violation found and fixed; a real garden-purge FK-survival bug found and fixed before it could ship                                                  | No publisher-facing web UI exists for ANY publication kind yet (greenfield, out of this AC's own backend-focused wording); no iOS client-portal; real time-lapse derivative generation deferred a third time, same infrastructure gap each time                                                                                                                                                          |
| P11-OBS-01    | Twelve real structured log events across the P11 feature surface, zero new queries; a real pre-existing prohibited-content leak found and fixed (`plant_species_ai.no_catalog_match`'s raw `commonName`); `taxon_enrichment.sweep_completed` gained `durationMs`; consent-boundary catalog tests; HTTP tests pinning the wire | No live Cloud Monitoring dashboard/log-based metric/alert policy deployed to any environment — the same posture every `-OBS-01` predecessor already states; identification ambiguity/model fallback/duplicate detection/upload recovery/comparison use/license rejection have no real feature behind them to instrument, named explicitly rather than invented around                                    |
| P11-QA-01     | This review                                                                                                                                                                                                                                                                                                                   | G10 approval itself — see below                                                                                                                                                                                                                                                                                                                                                                          |

## Section 20.5 exit criteria

- **Every add surface distinguishes an actual plant from a candidate without counting planned plants as present.** Yes — `plant_candidate` is a structurally separate table from `plant` on both platforms; `plants.actual_created`/`plants.candidate_added` (P11-OBS-01) even prove the distinction is live-observable in production telemetry, not merely a type-level claim.
- **Candidate conversion is explicit, idempotent, revision-aware, and history-preserving.** Yes — `ConvertCandidate` is `If-Match`-revision-guarded and idempotency-keyed (P11-API-01), proven against a genuine concurrent-conversion race in the P11-DATA-01/API-01 integration tests; `candidate_conversion` is an append-only provenance row, never an overwrite.
- **Plant creation succeeds during source outages; enrichment is asynchronous, deduplicated, provenance-preserving, license-filtered, refreshable, and visibly partial when incomplete.** Yes for the mechanism (zero registered providers is a documented, tested, honest no-op — `stoppedOnQuotaExhaustion`/`degradationReasons` make every failure mode visible, never a crash); partial for coverage (one of nine ADR-0016 sources has a real adapter today, the other eight are the identical shape away).
- **Search and every documented filter work against authorized server results and have complete native and web interaction states.** Partial — the filters that exist (query, status, priority, `identified`, lifecycleStage, groupingKind) are real, authorized, and have real native+web interaction states (P11-WEB-01/IOS-01); six more filter dimensions P11-SEARCH-01's own row names are scoped, documented follow-up, not built.
- **Plant library, add, detail, suitability, journal, comparison, and health-review workflows have the same product meaning on native and web.** Yes for what both platforms actually built this session (candidates list/add/detail/suitability/convert, health-suggestion review, `identified` filter) — both clients were built from the identical contract in the same pass, not independently reconciled after the fact; comparison itself has no backend endpoint on either platform (a documented P11-MEDIA-01/OBS-01 gap, not a native/web divergence).
- **Actual plants retain stable identity across repeated photo observations, map movement, renaming, re-identification, synchronization, and publication.** Yes — `plant.id` is immutable across every one of these operations by construction (revision-guarded updates, never a delete-and-recreate); P11-SHARE-01's own publication-kind work reused the exact same plant/observation identifiers without introducing a second identity concept.
- **Native journal capture survives offline operation, process termination, and interrupted upload without duplicating the observation.** Yes — `MediaUploadCoordinator`'s own iOS test suite (already passing before this window, reconfirmed in this session's full `swift test` run) explicitly covers relaunch recovery, an expired upload session, and a captured-but-never-registered transfer, each converging on exactly one observation.
- **Media licenses, attribution, EXIF privacy, derivatives, retention, deletion, export, and client entitlements pass.** Yes, with one real finding fixed in this window: P11-SHARE-01 found and closed a genuine gap where `isMediaClientSafe` accepted an ORIGINAL (non-derivative) media record, which could have exposed embedded EXIF/GPS to a client — now derivative-only, proven by a real rejection test. Licensed-image ingestion (a commercial vendor beyond the free baseline) remains the same owner-only gate P11-PROV-01 already names.
- **Health output remains an uncertain suggestion, can request more evidence, requires review, and cannot bypass recommendation safety policy.** Yes — `requiresConfirmation`/`requestedAdditionalEvidence`/`safetyClass`/the four-state `disposition` are all real, tested fields (P11-HEALTH-01); `PLANT_CONDITION_AI_ENABLED` stays `false` in every environment, so this ships real and kill-switched, the identical ADR-0015 posture Phase 10 already established and this phase deliberately did not re-litigate.
- **Large images and icons lead the presentation while accessibility, localization, keyboard, VoiceOver, Dynamic Type, contrast, reduced motion, and non-color meaning pass.** Partial — the AUTOMATED half passes: iOS's own `ArchitectureTests` accessibility-convention scan (no fixed frame dimension escapes `@ScaledMetric`, no empty-titled control, reduced-motion consulted, no hard-coded font size) is green in this window's fresh `swift test` run, and both clients ship complete `en`/`ru` localization catalogues with completeness tests. What this sandbox genuinely cannot produce is the MANUAL half named here — an actual screen-reader operator, a real device's Dynamic Type/contrast rendering, real keyboard-only navigation — the identical class of gap Phase 10's own review named for its own owner-only actions.
- **G10 is approved.** Not yet — approval is an owner action on this evidence, not a criterion this session's own work can satisfy by itself.

## Fresh verification run for this review

Not carried over from any individual work package's own prior notes — re-run in this same session,
after every above commit landed:

- `apps/ios`: `swift build` clean; `swift test` — **1008 tests, 142 suites, all green**, including
  the `ArchitectureTests` dependency-rule and accessibility-convention scans and the full
  `MediaUploadCoordinator` offline/relaunch/interrupted-upload suite.
- `pnpm -r test` (every workspace package — run fresh, post-commit, in this same session, no
  carried-over numbers): `services/api` **339 files, 2701 tests, all green** (no environmental
  flakiness this run, unlike several earlier work packages' own isolated Docker-contention retries);
  `apps/web` **126 files, 1029 tests, all green**; `services/workers` **22 files, 134 tests, all
  green**; `packages/api-contracts` **29/29**; `packages/geometry-contracts` **113/113**;
  `packages/test-fixtures` **21/21**.

## Assessment

Phase 11 is implementation-complete against every exit criterion this session's own work can satisfy
without an owner, a real device, or a deployed environment. Two real, previously-unknown bugs were
found and fixed while building and verifying this phase, not merely reported: the EXIF/GPS media-
safety gap (P11-SHARE-01) and the garden-purge foreign-key failure mode the SAME package's own new
schema would otherwise have introduced (also P11-SHARE-01) — both close before they could ever reach
production, both proven by new, real tests, not just patched and assumed fixed. One real, pre-
existing prohibited-content telemetry leak (P11-OBS-01's own `plant_species_ai.no_catalog_match`
finding) was also found and closed.

What remains is not unbuilt work disguised as a gate: it is the same category of owner-only/device-
only/deployment-only action Phase 10's own review already drew a clean line around, applied here to
a larger, thirteen-work-package phase — a manual accessibility pass with a real screen reader and
real devices; the eight remaining provider adapters and the human-review workflow that makes
fetched-but-unreviewed knowledge visible; a deployed Cloud Monitoring dashboard/alert policy (every
`-OBS-01` package in this codebase has deliberately never deployed one, by design, not by omission);
and the handful of scoped, named, forward-documented filter/UI dimensions each "first real pass"
work package explicitly declined to bundle in rather than dilute review quality. G10 itself remains
